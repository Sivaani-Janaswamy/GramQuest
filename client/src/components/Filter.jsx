...
const Filter = ({ onFilterChange }) => {
  ...
  const handleFilterChange = (filter) => {
    ...
    // Trigger the filtering in the parent component
    if (filter === 'upvotes') {
      onFilterChange({ filterByUpvotes: true, filterByStars: false, sortByDate: false, sortByOldest: false });
    } else if (filter === 'stars') {
      onFilterChange({ filterByUpvotes: false, filterByStars: true, sortByDate: false, sortByOldest: false });
    } else if (filter === 'recent') {
      onFilterChange({ filterByUpvotes: false, filterByStars: false, sortByDate: true, sortByOldest: false });
    } else if (filter === 'oldest') {
      onFilterChange({ filterByUpvotes: false, filterByStars: false, sortByDate: false, sortByOldest: true });
    } else {
      onFilterChange({ filterByUpvotes: false, filterByStars: false, sortByDate: false, sortByOldest: true });
    }
  };
